...
Tracker.prototype.updateEmployeeRoleTracker = function() {
    ...
    function updateEmployee(name) {
        inquirer.prompt ([
            {
                type: 'list',
                name: 'job',
                message: 'What would you like to update this employees role too?',
                choices: [
                    'Accounting Manager', 
                    'Accounts Payable Clerk', 
                    'Bookkeeper', 
        ...
        .then((response) => {
            ...
            const sql = `UPDATE employees SET job_id = ${num_job_id}, department_id = ${num_department_id}, manager_name = '${str_manager_name}' WHERE first_name = '${name}'`;
        
            db.query(sql, (err, rows) => {
                if (err) throw err;
                console.log("Employee has been updated.");
                Tracker.prototype.initializeTracker(); 
